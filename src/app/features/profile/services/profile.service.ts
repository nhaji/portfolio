...
export class ProfileService extends HttpService {
  ...
  getProfileSummary(): Observable<ProfileModel> {
    return this.get<ProfileModel>(`/profile-summary.json`)
    .pipe(delay(this.artificialDelayMs)); 
  }

  getExperiences(): Observable<ExperienceModel[]> {
    return this.get<ExperienceModel[]>(`/experiences.json`)
    .pipe(delay(this.artificialDelayMs));
  }

  getContacts(): Observable<ContactModel[]> {
    return this.get<ContactModel[]>(`/contacts.json`)
    .pipe(delay(this.artificialDelayMs));
  }

  getKeyPoints(): Observable<KeyPointModel[]> {
    return this.get<KeyPointModel[]>(`/key-points.json`)
    .pipe(delay(this.artificialDelayMs));
  }

  getLanguages(): Observable<LanguageModel[]> {
    return this.get<LanguageModel[]>(`/languages.json`)
    .pipe(delay(this.artificialDelayMs));
  }

  getSkills(): Observable<SkillModel[]> {
    return this.get<SkillModel[]>(`/skills.json`)
    .pipe(delay(this.artificialDelayMs));
  }

  getStudies(): Observable<StudyModel[]> {
    return this.get<StudyModel[]>(`/studies.json`)
    .pipe(delay(this.artificialDelayMs));
  }
}
